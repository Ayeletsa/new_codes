function plot_co_main_cell_fig_inter_field(dir_param_file_name,population_param_file_name)
    ...
    if isnan(behavior_struct.co(1).inter_field_firing_rate.inter_field_edge) & isnan(behavior_struct.co(2).inter_field_firing_rate.inter_field_edge)
        ...
        if exist(file_name)
            ...
            for bin_dis_i=1:size(behavior_struct.co(1).firing_rate.dis_bin_size,1)
                ...
                if behavior_struct.exp_data.mean_fr==0
                    behavior_struct.exp_data.mean_fr=eps;
                end
                ...
                for ii_dir = 1:2
                    ...
                    if ~isnan(behavior_struct.co(ii_dir).inter_field_firing_rate.inter_field_edge)
                        num_fileds=size(behavior_struct.co(ii_dir).inter_field_firing_rate.inter_field_edge,2);
                        
                        %end
                        plot(behavior_struct.solo(ii_dir).field_height,behavior_struct.solo(ii_dir).field_center,'*r')
                        
                        for fr_i= 1:size(behavior_struct.solo(ii_dir).field_edges,2)
                        ...
                        for fr_i= 1:size(behavior_struct.co(ii_dir).inter_field_firing_rate.inter_field_edge,2)
                            plot([-0.5 -0.5],[behavior_struct.co(ii_dir).inter_field_firing_rate.inter_field_edge(1,fr_i),behavior_struct.co(ii_dir).inter_field_firing_rate.inter_field_edge(2,fr_i)],'color',[0 0 1],'linewidth',2)
                        end
                        ...
                        if num_fileds>0
                            count=0;
                            for fr_i=1:num_fileds
                                if ~isnan(behavior_struct.co(ii_dir).inter_field_firing_rate.information_per_spike_per_field{fr_i} )
                                    count=count+1;
                                    axes('units','normalized','Position',[0.32+dir_adj y_pos(1)+dis_y_pos*(count-1) 0.12 0.05]);
                                    
                                    bins_center=behavior_struct.co(ii_dir).firing_rate.dis_X_bins_vector_of_centers{bin_dis_i,bin_allo_i};
                                    shuf_data=behavior_struct.co(ii_dir).inter_field_firing_rate.signif_field{1, fr_i}.shuffled_data;
                                    r=behavior_struct.co(ii_dir).inter_field_firing_rate.dis_x_fr_per_field{1, fr_i};
                                    ...
                                    if behavior_struct.co(ii_dir).inter_field_firing_rate.signif_field{1, fr_i}.signif_based_on_extreme_bins==1
                                        neg_signif=behavior_struct.co(ii_dir).inter_field_firing_rate.signif_field{1, fr_i}.neg_signif;
                                        plot(bins_center(neg_signif),r(neg_signif)+1,'r*')
                                        pos_signif=behavior_struct.co(ii_dir).inter_field_firing_rate.signif_field{1, fr_i}.pos_signif;
                                        plot(bins_center(pos_signif),r(pos_signif)+1,'g*')
                                    end
                                    max_y = round(max(max([r;shuf_data]))*1.1*10) / 10 +1;
                                    if max_y < 1 | isnan(max_y)
                                        max_y = 1;
                                    end
                                    bins_diff = mean(diff(co_shuffle_struct(ii_dir).shuffled_data.ego_bin_centers));
                                    x_limits = [co_shuffle_struct(ii_dir).shuffled_data.ego_bin_centers(1)-bins_diff/2 co_shuffle_struct(ii_dir).shuffled_data.ego_bin_centers(end)+bins_diff/2];
                                    ...
                                    cv=std_r/mean_r;
                                    if behavior_struct.co(ii_dir).inter_field_firing_rate.signif_field{1, fr_i}.signif_based_on_extreme_bins==1
                                        text(x_limits(2),max_y*0.5,sprintf('SI=%.2f sig bins \n #spikes=%d',behavior_struct.co(ii_dir).inter_field_firing_rate.information_per_spike_per_field{fr_i},behavior_struct.co(ii_dir).inter_field_firing_rate.number_of_spikes_per_field{fr_i}),'color',[1 0 0])
                                    else
                                        text(x_limits(2),max_y*0.5,sprintf('SI=%.2f \n #spikes=%d',behavior_struct.co(ii_dir).inter_field_firing_rate.information_per_spike_per_field{fr_i},behavior_struct.co(ii_dir).inter_field_firing_rate.number_of_spikes_per_field{fr_i}))
                                        
                                    end
                                    if behavior_struct.co(ii_dir).inter_field_firing_rate.signif_field{1, fr_i}.signif_based_on_CV==1
                ...
                fig_name=fullfile(co_fig_folder_name,['inter_field_cell_',num2str(cell_num),'_day_',num2str(day),'_bat_',num2str(bat),'.png']);
                saveas(gcf,fig_name)
...
%             if signif==1
%                 fig_name=fullfile(co_signif_cells_fig_folder_name,['inter_field_cell_',num2str(cell_num),'_day_',num2str(day),'_bat_',num2str(bat),'.png']);
%                 saveas(gcf,fig_name)
%             end
            clf(gcf)
