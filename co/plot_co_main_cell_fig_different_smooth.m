function plot_co_main_cell_fig_different_smooth(dir_param_file_name,population_param_file_name)
...
for ii_cell = 3:length(behavior_struct_names)
    ...
    cell_num=behavior_struct.exp_data.cell_num;
     if ~isempty(behavior_struct.solo(1).field_height) | ~isempty(behavior_struct.solo(2).field_height)
    ...
    if exist(file_name)
        ...
        for bin_dis_i=1:size(behavior_struct.co(1).firing_rate.dis_bin_size,1)
                ...
                if num_fileds>0
                    ...
                    for fr_i=sort(fields_to_plot)
                        ...
                        if sum(~isnan(behavior_struct.co(ii_dir).firing_rate.dis_x_fr_per_field{1, fr_i}))~=0
                            ...
                            for r_i=1:3
                            r=behavior_struct.co(ii_dir).firing_rate.dis_x_fr_per_field_new_smooth_smooth_window{1, fr_i}(r_i,:);
                            plot(bins_center ,r,'LineWidth',lwidth);
                            end
            ...
            fig_name=fullfile(co_fig_folder_name,['diferent_smooth_cell_',num2str(cell_num),'_day_',num2str(day),'_bat_',num2str(bat),'.png']);
            saveas(gcf,fig_name)
        ...
        if signif==1
            fig_name=fullfile(co_signif_cells_fig_folder_name,['diferent_smooth_cell_',num2str(cell_num),'_day_',num2str(day),'_bat_',num2str(bat),'.png']);
            saveas(gcf,fig_name)
        end
        clf(gcf)
