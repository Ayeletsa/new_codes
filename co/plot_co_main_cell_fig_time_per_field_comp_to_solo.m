function plot_co_main_cell_fig_time_per_field_comp_to_solo(dir_param_file_name,population_param_file_name,co_param_file_name,per_field_param_file_name)
...
co_time_fig_folder_name='D:\Ayelet\2bat_proj\Analysis\new_code\figures\per_field_time_analysis_comp_to_solo\';
                    ...
                    if length(fields_to_plot)>0
                        ...
                        for fr_i=fields_to_plot
                            ...
                            if max(ind_length)>=min_r_length_per_field*length(r) & per_field(fr_i).number_of_spikes_per_field>min_n_spike_per_field
                                ...
                                shuf_data=per_field(fr_i).solo_interp_time_tuning_shuffle;
                              ...
                              %  prc_50_solo=prctile(per_field(fr_i).solo_firing_rates_time_bins,50);
                               % mean_solo=nanmean(per_field(fr_i).solo_firing_rates_time_bins);
                                ...
                                prct_alpha_max=prctile(shuf_data,100-benf_correction);
                                prct_alpha_min=prctile(shuf_data,benf_correction);
                                pos_signif=find(r>prct_alpha_max);
                                neg_signif= find(r<prct_alpha_min);
                                plot(bins_center(neg_signif),r(neg_signif)+1,'ro','markersize',8)
                                plot(bins_center(pos_signif),r(pos_signif)+1,'go','markersize',8)
