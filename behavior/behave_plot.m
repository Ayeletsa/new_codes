function behave_plot(general_behavior_data_file_name,bat,day,CO_point,removed_co,behav_modes_plot,correction_type)
...
title(sprintf('%s - bat %d day %s',correction_type, bat,day), 'FontSize', 11, 'FontWeight', 'Bold');
box off;
axis off;
% else
%     color_self=[1 0 0];
...
for time_i=1:length(time_vec_for_plot)-1
 ...
 for behav_mod_i=1:length(behav_modes_plot)
    ...
    %% plot co:
if ~isempty(CO_point)    
    co_for_plot=CO_point(ts(CO_point)>=time_vec_for_plot(time_i) & ts(CO_point)<=time_vec_for_plot(time_i+1));
    plot(ts(co_for_plot),pos_self_x(co_for_plot),'ko')
    
end

if ~isempty(removed_co)    
    co_for_plot=removed_co(ts(removed_co)>=time_vec_for_plot(time_i) & ts(removed_co)<=time_vec_for_plot(time_i+1));
    plot(ts(co_for_plot),pos_self_x(co_for_plot),'m*')
