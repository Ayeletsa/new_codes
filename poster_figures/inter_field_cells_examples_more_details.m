...
cells_num_dir=[298,2;296,2;274,2;251,2;238,1;235,2;226,1;211,1;531,1;525,1;516,1;494,1;474,1;456,1;808,2;777,1;764,1;725,1;719,1;...
    713,2;695,2;648,2;639,1;782,1;724,1];
relevant_field{1}=[3];
relevant_field{2}=[2,3];
relevant_field{3}=[2];
relevant_field{4}=[1];
relevant_field{5}=[1];
relevant_field{6}=[2,3];
relevant_field{7}=[1,2];
relevant_field{8}=[2];
relevant_field{9}=[1];
relevant_field{10}=[1];
relevant_field{11}=[2,3];
relevant_field{12}=[2];
relevant_field{13}=[2];
relevant_field{14}=[1];
relevant_field{15}=[3];
...
for ii_cell = 1:length(cells_num_dir)
    ...
    if exist(file_name)
        ...
        text(-0.05,0.8,sprintf('Cross-over data\nwithin the "inter-fields"'),'fontsize',fsize,'fontweight','bold')
        box off
        axis off
         %% 2D raster CO
        axes(co_raste_ax)
      ...
      normalize_to_other_max=[];
        ...
        X_bins_vector=dis_X_bins_vector_2D;
        X_bins_vector_of_centers=dis_X_bins_vector_of_centers_2D;
        Y_bins_vector=allo_X_bins_vector_2D;
        Y_bins_vector_of_centers=allo_X_bins_vector_of_centers_2D;
        allo_bin_size=allo_X_bin_size_2D;
        dis_bin_size=dis_X_bin_size_2D;
        ...
        inter_field=behavior_struct.co(ii_dir).inter_field;
        per_field=behavior_struct.co(ii_dir).per_field_href;
        x=[behavior_struct.solo(ii_dir).fields.edges_href];
        ...
        plot(repmat([time_limits(1);time_limits(1)],1,size(field_edges,2)),field_edges,'color',[.5 .5 .5],'linewidth',2)
        plot(repmat([time_limits(1);time_limits(1)],1,size(inter_field_edges,2)),inter_field_edges,'color',[0 0 1],'linewidth',2) 
        plot(time_limits(1)*ones(1,length(fields_to_plot)),mean(inter_field_edges(:,fields_to_plot)),'r.')
        ...
        if num_fileds>0
            ...
            for fr_i=sort(fields_to_plot)
                ...
                if sum(~isnan(behavior_struct.co(ii_dir).inter_field(fr_i).tuning_dis_x_fr_per_field))~=0
                    bins_center=dis_per_field_bin_vec_of_center;
                    shuf_data=behavior_struct.co(ii_dir).inter_field(fr_i).dis_signif_field.shuffled_data;
                    r=behavior_struct.co(ii_dir).inter_field(fr_i).tuning_dis_x_fr_per_field ;
                    ...
                    if behavior_struct.co(ii_dir).inter_field(fr_i).dis_signif_field.signif_based_on_extreme_bins==1
                        neg_signif=behavior_struct.co(ii_dir).inter_field(fr_i).dis_signif_field.neg_signif;
                        plot(bins_center(neg_signif),r(neg_signif),'ro')
                        pos_signif=behavior_struct.co(ii_dir).inter_field(fr_i).dis_signif_field.pos_signif;
                        plot(bins_center(pos_signif),r(pos_signif),'go')
                    end
    ...
    fig_name=fullfile(dir_name,['inter_field_egocentric_cell_',num2str(cell_num),'_dir_',num2str(ii_dir)]);
